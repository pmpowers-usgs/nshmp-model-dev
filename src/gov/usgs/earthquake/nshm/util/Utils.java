...
public class Utils {
  ...
  /**
   * Converts mblg magnitude to moment magnitude (M<sub>w<sub>) according to the
   * conversion required by the supplied fault <code>code</code>.
   ...
   * @return the converted magnitude or the input mag if code is not applicable
   *         or <code>null</code>
   * @see FaultCode
   */
   ...
   * 0.0s), method returns <code>Math.min(ln(1.5g), gnd)</code>; for <code>0.02s
   * &lt; period &lt; 0.5s</code>, method returns <code>Math.min(ln(3.0g),
   * gnd)</code>. This is used to clip the upper tail of the exceedance curve.
...
// @formatter:off
  ...
  /**
   * Returns the epistemic ground-motion uncertainty for the supplied magnitude
   * (M) and distance (D).
   * @param M magnitude
   * @param D distance
  ...
  /**
   * Returns the URL of an NSHMP resource. Path should be specified as absolute
   * from NSHMP resource directory (e.g. "/sources/WUS/...").
  ...
  public static FocalMech typeForID(int id) {
    return id == 1 ? FocalMech.STRIKE_SLIP
        : id == 2 ? FocalMech.REVERSE : id == 3 ? FocalMech.NORMAL : null;
  }
   ...
   * reading across. The nodes in OpenSHA <code>GriddedRegion</code>s are stored
   * from bottom left to top right, also reading across. This method places
   * values at their proper index. <i><b>NOTE</b></i>: NSHMP binary grid files
   * are all currently little-endian. The grid files in some other parts of the
   * USGS seismic hazard world are big-endian. Beware.
   * @param url to read
   * @param nRows
   * @param nCols
  ...
  /**
   * Custom method to read NSHMP CEUS craton and margin files. These are fortran
   * output logical files and have 4 f-specific bytes on each end of the file.
   * Each logical fills 4bytes and although the files contain 4*128000 bytes,
   * the CEUS mMax files are used in cra.f when generating so only 4*127755 are
   * filled. The reamining slots are false and not considered.
   * @param url to read
   * @param nRows
   * @param nCols
   * @return a 1D array of appropriately ordered values
  ...
  /*
   * Alternate more compact form for Location.toString(). Strips trailing zeros
   * for NSHMP grid nodes which all have 0.1deg spacing.
   */
