...
class SystemFaultSurface {
	...
	private void init() {
		...
		dist[0] = Locations.horzDistanceFast(surf1.getUpperEdge().first(), surf2.getUpperEdge().first());
		...
		for(int i=1; i< surfaces.size()-1; i++) {
			...
			double d1 = Locations.horzDistanceFast(surf1.getUpperEdge().last(), surf2.getUpperEdge().first());
			double d2 = Locations.horzDistanceFast(surf1.getUpperEdge().last(), surf2.getUpperEdge().last());
			if(d1<d2)
